...
#define ALLOC_CONNECTIONS_ONCE 256
	...
	//分配内存给 g_nio_thread_data  one thread one thread_data
	...
	pDataEnd = g_nio_thread_data + g_work_threads;//g_work_threads:默认为4个线程,指针加数字,按数组理解
	for (pThreadData=g_nio_thread_data; pThreadData<pDataEnd; pThreadData++)
	{
		//初始化工作线程对应的事件(IOEventPoller,size - 256+2,timeout-1000,extra_events-0)
		 ...
		 * 管道是一种把两个进程之间的标准输入和标准输出连接起来的机制，从而提供一种让多个进程间通信的方法，当进程创建管道时，每次
			...
			pipe 参考 http://www.cnblogs.com/kunhu/p/3608109.html
		 * */
		//创建一条管道,如果创建成功,则将两个文件描述符放入pThreadData->thread_data.pipe_fds中,也就是说文件描述符是由PIPE create
		...
		//创建线程,设置线程执行的入口,相当于 Thread run method
		/**
		 * 	第一个参数为指向线程标识符的指针。
　　			第二个参数用来设置线程属性。
　　			第三个参数是线程运行函数的起始地址。
　　			最后一个参数是运行函数的参数。
			...
			参考: http://blog.csdn.net/liangxanhai/article/details/7767430
		 * */
...
/**
 * Acceptor线程入口  arg  ServerSocket address
 * */
static void *accept_thread_entrance(void* arg)
{
	...
	while (g_continue_flag)
	{
		...
		//将任务的arg参数转换为StorageClientInfo来处理数据.之后对StorageClientInfo处理就是对task的arg进行处理
		pClientInfo = (StorageClientInfo *)pTask->arg;
		//设置客户端句柄
		pTask->event.fd = incomesock;
		//客户请求状态设置为0
		pClientInfo->stage = FDFS_STORAGE_STAGE_NIO_INIT;
		//fd % 工作线程数量交由哪个工作线程进行处理
		...
		// get task memery address
		task_addr = (long)pTask;
		// write task by work thread pipe[1],work thread loop handle pipe[0] event

		// accept thread write --->>> work thread read
		if (write(pThreadData->thread_data.pipe_fds[1], &task_addr, \
			sizeof(task_addr)) != sizeof(task_addr))
		{
			close(incomesock);
 ...
 * 工作线程入口
 *
 * arg : struct storage_nio_thread_data *pThreadData;
 * */
static void *work_thread_entrance(void* arg)
{
	int result;
	...
	//之前已初始化过IOEVENT,现在进行WORK_THREAD的循环处理
...
static int storage_upload_file(struct fast_task_info *pTask, bool bAppenderFile)
{
	...
	//取文件扩展名 6 bytes  file_ext_name init 16 + 1 bytes
	...
	*(file_ext_name + FDFS_FILE_EXT_NAME_MAX_LEN) = '\0';
	...
	pFileContext->calc_crc32 = true;//need crc32 validate
	pFileContext->calc_file_hash = g_check_file_duplicate;//validate repeate file default false
	pFileContext->extra_info.upload.start_time = g_current_time;//record current time

	strcpy(pFileContext->extra_info.upload.file_ext_name, file_ext_name);//set file ext name
	storage_format_ext_name(file_ext_name, \
			pFileContext->extra_info.upload.formatted_ext_name);//fommat file ext name
	pFileContext->extra_info.upload.trunk_info.path. \
				store_path_index = store_path_index;//set store_path_index
	pFileContext->extra_info.upload.file_type = _FILE_TYPE_REGULAR;//set file type
	pFileContext->sync_flag = STORAGE_OP_TYPE_SOURCE_CREATE_FILE;//set storage opt
	pFileContext->timestamp2log = pFileContext->extra_info.upload.start_time;
	pFileContext->op = FDFS_STORAGE_FILE_OP_WRITE;//set context op is FDFS_STORAGE_FILE_OP_WRITE
	if (bAppenderFile)
	{
	...
	if (pFileContext->extra_info.upload.file_type & _FILE_TYPE_TRUNK)
	{
		...
		pFileContext->extra_info.upload.if_sub_path_alloced = true;//alloc by who?
		pTrunkInfo = &(pFileContext->extra_info.upload.trunk_info);//?
		if ((result=trunk_client_trunk_alloc_space( \
			TRUNK_CALC_SIZE(file_bytes), pTrunkInfo)) != 0)
		{
