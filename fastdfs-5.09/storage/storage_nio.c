...
/**通知有读事件
 * pEntry->fd, event, pEntry->timer.data
 * */
void storage_recv_notify_read(int sock, short event, void *arg)
{
	...
	while (1)
	{
		...
		//由accept thread by work thread pipe[1] 传过来的Task address
...
static int storage_nio_init(struct fast_task_info *pTask)
{
	...
	//change stage to FDFS_STORAGE_STAGE_NIO_RECV-1
...
/**
 * input  1-  pTask->event.fd,IOEVENT_READ, pTask
 *
 * */
static void client_sock_read(int sock, short event, void *arg)
{
	...
	while (1)
	{
		...
		*/
		...
		if (pClientInfo->total_length == 0) //header
		{
			...
			{
				//接收的数据不足10字节,task记录offset为已经接收的字节数然后返回
				pTask->offset += bytes;
				return;
			}
			//获得真实数据的总长度->现在不包含头部
			...
			//总数据长度重新加上头部的长度则是此次请求的总长度
			pClientInfo->total_length += sizeof(TrackerHeader);
			if (pClientInfo->total_length > pTask->size)
			{
				//如果请求总长度 > Task的大小(256K?确认一下),则当次TASK处理的length为Task size
				pTask->length = pTask->size;
			}
			else
			{
